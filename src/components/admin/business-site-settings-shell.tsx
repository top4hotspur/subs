...
type Props = {
  initialSlug?: WebsiteTemplateSlug;
};

export function BusinessSiteSettingsShell({ initialSlug }: Props) {
  const [selectedSlug, setSelectedSlug] = useState<WebsiteTemplateSlug>(initialSlug ?? firstSlug());
  ...
  function load(slug: WebsiteTemplateSlug): void {
  ...
  function updateServices(services: SiteServiceItem[]): void {
    if (!template) {
      return;
    }
  ...
  function updateStaff(nextStaff: StaffMember[]): void {
    const saved = saveLocalStaff(selectedSlug, nextStaff);
    setStaff(saved);
    setStatusText("Staff saved locally.");
  }

  function updateRoles(nextRoles: StaffRoleDefinition[]): void {
    const saved = saveLocalStaffRoles(selectedSlug, nextRoles);
    setRoles(saved);
    setStatusText("Staff roles saved locally.");
  }

  function updateNotificationTemplates(nextTemplates: NotificationTemplate[]): void {
