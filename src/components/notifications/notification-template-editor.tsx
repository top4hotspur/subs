  ...
  const grouped = useMemo(
    () =>
      events
        .map((eventType) => ({
          eventType,
          items: templates.filter((template) => template.eventType === eventType),
        }))
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-600">Email notifications are included. WhatsApp templates are optional (+£10/month add-on).</p>
            ...
            <div className="mt-3 space-y-3">
              {group.items.map((template) => {
                const whatsappLocked =
                  template.channel === "WHATSAPP" && !whatsappAddonEnabled;
                  ...
                  <div key={template.id} className="rounded-lg border border-slate-200 bg-white p-3">
                    ...
                    {whatsappLocked ? (
                      <p className="mt-2 text-xs text-amber-700">
                        WhatsApp template is currently disabled because the optional WhatsApp add-on is not enabled.
                      </p>
                    ) : null}
                  </div>
                );
