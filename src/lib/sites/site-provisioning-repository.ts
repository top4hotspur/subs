...
export async function createSiteDomain(input: CreateOrUpdateSiteDomainInput) {
  ...
  const record = await prisma.siteDomain.upsert({
    ...
    create: {
      ...
      status: parsed.status,
      domainStatus: parsed.domainStatus,
      domainSetupMode: parsed.domainSetupMode,
      dnsStatus: parsed.dnsStatus,
      sslStatus: parsed.sslStatus,
      nameserverInstructionsSentAt: parsed.nameserverInstructionsSentAt,
      dnsLastCheckedAt: parsed.dnsLastCheckedAt,
      dnsVerifiedAt: parsed.dnsVerifiedAt,
      goLiveRequestedAt: parsed.goLiveRequestedAt,
      wentLiveAt: parsed.wentLiveAt,
      domainNotes: parsed.domainNotes,
      expectedDnsTarget: parsed.expectedDnsTarget,
      expectedNameservers: parsed.expectedNameservers === undefined ? undefined : toJson(parsed.expectedNameservers),
      lastDnsCheckResult: parsed.lastDnsCheckResult === undefined ? undefined : toJson(parsed.lastDnsCheckResult),
