      ...
      <section className="mx-auto max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5">
          <Suspense fallback={<p className="text-sm text-slate-600">Loading registration...</p>}>
            <SiteCustomerAccountForm siteSlug={site.slug} mode="register" />
          </Suspense>
        </div>
        <p className="mt-4 text-sm text-slate-600">
          Already have an account?{" "}
          <Link className="font-semibold text-teal-700 underline" href={`/sites/${encodeURIComponent(site.slug)}/account/login`}>
            Log in here
          </Link>.
        </p>
      </section>
