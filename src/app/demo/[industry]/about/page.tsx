...
type DemoIndustryAboutPageProps = {
...
export default async function DemoIndustryAboutPage({ params }: DemoIndustryAboutPageProps) {
  const { industry } = await params;
  if (!isWebsiteTemplateSlug(industry)) notFound();
  const template = getWebsiteTemplate(industry);
  if (!template) notFound();
  return <DemoAboutContactPage template={template} mode="about" />;
}
