...
export async function GET(request: Request) {
  ...
  if (!code) {
    return NextResponse.json({ ok: false, error: "MISSING_SQUARE_OAUTH_CODE" }, { status: 400 });
  }
  return NextResponse.json({
    ok: false,
    error: "SQUARE_OAUTH_TOKEN_EXCHANGE_NOT_IMPLEMENTED",
    message: "Square returned an authorization code, but secure token exchange/storage is intentionally not enabled in this pass.",
  }, { status: 501 });
}
