...
export async function DELETE(
  ...
  try {
    ...
    const current = await getCustomerSiteSettings(resolved.tenantSiteId);
    if (current?.logoStorageKey) {
      await deleteSiteMediaObject({ key: current.logoStorageKey });
    }
    ...
    return NextResponse.json({ ok: false, error: "SITE_ADMIN_LOGO_DELETE_FAILED", message }, { status: 500 });
  }
}
